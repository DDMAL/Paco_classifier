...
def getTrain(input_image, gt, patch_height, patch_width):
    ...
    for h in range(0, input_image.shape[0] - patch_height, hstride):
        for w in range(0, input_image.shape[1] - patch_width, wstride):
            x_sample = input_image[h:h + hspan, w:w + wspan]
            ...
            for label in gt:
                Y_train[label].append(gt[label][h:h + hspan, w:w + wspan])
...
if __name__ == "__main__":
    input_path = '/Users/jcalvo/Desktop/McGill/dama-corpora/Corpora/Layers/Salzinnes/CF-009/'

    # Read original image
    input_image = cv2.imread(input_path + '/CF-009.jpg', True)  # 3-channel

    # Open layers as expected
    background = cv2.imread(input_path + '/CF-009_Decoration.jpg',cv2.IMREAD_UNCHANGED)  # 4-channel
    notes = cv2.imread(input_path + '/CF-009_Notes.jpg',cv2.IMREAD_UNCHANGED)  # 4-channel
    lines = cv2.imread(input_path + '/CF-009_Staff.jpg',cv2.IMREAD_UNCHANGED)  # 4-channel
    text = cv2.imread(input_path + '/CF-009_Text.jpg',cv2.IMREAD_UNCHANGED)  # 4-channel

    gt = {}
    gt['symbols'] = (notes[:, :, 3] == 255)
    gt['staff'] = (lines[:, :, 3] == 255)
    gt['text'] = (text[:, :, 3] == 255)
    gt['background'] = (background[:, :,3] == 255)  # background is already restricted to the selected regions (based on Pixel.js' behaviour)

    # Indicate paths
    output = {}
    path = '/Users/jcalvo/Desktop/McGill/Models/'
    output['symbols'] = path + '_symbols.hdf5'
    output['staff'] = path + '_staff.hdf5'
    output['text'] = path + '_text.hdf5'
    output['background'] = path +'bg.hdf5'

    # Call train_msae
    train_msae(input,gt,256,256,output,epochs=3)
