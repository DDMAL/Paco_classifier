...
def preprocess(config):
    ...
    return inputs, outputs, output_models_path, input_ports, output_ports

def profileDriver(output_file, steps, generator, config):
    print ("Write to {}".format(output_file))

    @helper.profile(output_file=output_file, lines_to_print=None, strip_dirs=True, sort_by='time')
    def dummyRun(_generator, steps):
        for x, y in _generator:
            steps-=1
            if steps==0:
                break

    print ("Start profiling with batch size {} and {} steps".format(config.batch_size, steps))
    dummyRun(generator, steps)
    print ("End profiling, dump to {}".format(output_file))

if __name__ == "__main__":
    # Do not touch these two lines.
    config = menu()
    inputs, outputs, output_models_path, input_ports, output_ports = preprocess(config)

    # All the profile stuff starts here
    generators = training.getTrain(inputs, input_ports, 
                                config.patch_height, config.patch_width, 
                                config.batch_size, config.file_selection_mode, config.sample_extraction_mode)

    import Paco_classifier.helper as helper
    for idx in range(len(generators)):
        steps=32
        output_file = "generator_b-{}_steps-{}_{}.profile".format(config.batch_size, steps, idx)
        profileDriver(output_file=output_file, steps=steps, generator=generators[idx], config=config)